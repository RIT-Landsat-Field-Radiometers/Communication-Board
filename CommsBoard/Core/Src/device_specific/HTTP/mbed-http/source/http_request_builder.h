...
public:
    ...
    char *build(const void *body, uint32_t body_size, uint32_t &size, bool skip_content_length = false)
    {
        ...
        size += strlen(method_str) + 1 + strlen(parsed_url->path()) +
                (strlen(parsed_url->query()) ? strlen(parsed_url->query()) + 1 : 0) + 1 + 8 + 2;
        ...
        req += strlen(method_str) + 1 + strlen(parsed_url->path()) +
               (strlen(parsed_url->query()) ? strlen(parsed_url->query()) + 1 : 0) + 1 + 8 + 2;
...
private:
    bool has_header(const char *key, const char *value = nullptr)
    {
        ...
        {
            if (strcmp(item.first.c_str(), key) == 0)
            { // key matches
                if (value == nullptr || (strcmp(item.second.c_str(), value) == 0))
                { // value is NULL or matches
                    return true;
                }
            }
            return false;
