...
public:
    ...
    */
    HttpRequest(http_method method, const char* url, const std::function<void(const char *at, uint32_t length)>& bodyCallback = nullptr)
...
//        _we_created_socket = true;
     ...
     * @param[in] bodyCallback Callback on which to retrieve chunks of the response body.
                                ...
                                which might use lots of memory.
    */
    HttpRequest(int32_t socketfd, http_method method, const char* url, const std::function<void(const char *at, uint32_t length)>& bodyCallback = nullptr)
...
protected:
    ...
    nsapi_error_t connect_socket(char *host, uint16_t port) override {
        ...
        com_sockaddr_t addr;
        auto err = com_gethostbyname(reinterpret_cast<const com_char_t *>(host), &addr);
        ...
        address.sin_port = COM_HTONS(80);
        address.sin_addr.s_addr = ((com_sockaddr_in_t*) &addr)->sin_addr.s_addr;

        err = com_connect(_socket, (com_sockaddr_t*) &address,
