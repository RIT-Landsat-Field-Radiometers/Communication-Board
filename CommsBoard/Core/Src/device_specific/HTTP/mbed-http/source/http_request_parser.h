...
public:
    ...
    HttpParser(HttpResponse* a_response, http_parser_type parser_type, std::function<void(const char *at, uint32_t length)>  a_body_callback = nullptr)
        : response(a_response), body_callback(std::move(a_body_callback))
    {
...
private:
    ...
    std::function<void(const char *at, uint32_t length)> body_callback;
    http_parser* parser;
