...
#include "../../../HTTP/mbed-http/http_parser/http_parser.h"
#include "../../../HTTP/mbed-http/source/http_parsed_url.h"
#include "../../../HTTP/mbed-http/source/http_request_builder.h"
#include "../../../HTTP/mbed-http/source/http_request_parser.h"
...
public:
    HttpRequestBase(int32_t socketfd, std::function<void(const char *at, uint32_t length)> bodyCallback)
            : _socket(socketfd), _body_callback(std::move(bodyCallback)), _request_buffer(NULL), _request_buffer_ix(0)
    {}
...
//            delete _socket;
     ...
     *         See get_error() for the error code.
     */
    HttpResponse *send(const std::function<const void *(uint32_t *)> &body_cb)
    {
     ...
     */
    [[nodiscard]] nsapi_error_t get_error() const
    {
        return _error;
    }
     ...
     */
    [[nodiscard]] size_t get_request_log_buffer_length() const
    {
...
private:
    nsapi_error_t connect_socket()
    {
        if (_response != nullptr)
        {
            ...
            return NSAPI_ERROR_IS_CONNECTED; // @todo, make a lookup table with errors
        }
    ...
    nsapi_size_or_error_t send_buffer(char *buffer, uint32_t buffer_size)
    {
        ...
        while (total_send_count < buffer_size)
        {
            ...
            nsapi_size_or_error_t send_result = com_send(_socket, reinterpret_cast<const com_char_t *>(buffer_slice), buffer_slice_size, COM_MSG_WAIT);//_socket->send(buffer_slice, buffer_slice_size);
...
//        while ((recv_ret = _socket->recv(recv_buffer, HTTP_RECEIVE_BUFFER_SIZE)) > 0)
        while ((recv_ret = com_recv(_socket, recv_buffer, HTTP_RECEIVE_BUFFER_SIZE, COM_MSG_WAIT)) > 0)
        {
...
private:
    int32_t _socket;
    std::function<void(const char *at, uint32_t length)> _body_callback;
